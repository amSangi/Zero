...
std::unique_ptr<MeshData> MeshGenerator::GenerateSphere(const Sphere& sphere)
{
    ...
    float latitude_step = math::kPi / static_cast<float>(latitude_count);
    float longitude_step = math::kTwoPi / static_cast<float>(longitude_count);

    // Generate vertices
    for (uint32 i = 0; i <= latitude_count; ++i)
    {
        // starting from pi/2 to -pi/2
        float latitude_angle = math::kHalfPi - static_cast<float>(i) * latitude_step;
        ...
        for (uint32 j = 0; j <= longitude_count; ++j)
        {
            ...
            vertex.texture_coordinate_ = math::Vec2f(static_cast<float>(j) / static_cast<float>(longitude_count),
                                                     static_cast<float>(i) / static_cast<float>(latitude_count));
...
std::unique_ptr<MeshData> MeshGenerator::GeneratePlane(const Plane& plane)
{
    std::vector<Vertex> vertices;
    std::vector<uint32> indices;
    ...
    {
        auto u_component = u_axis * static_cast<float>(i);
        float u_texture_coordinate = static_cast<float>(i) / static_cast<float>(plane.width_);
        for (uint32 j = 0; j <= plane.height_; ++j)
        {
            Vertex vertex{};
            vertex.position_ = u_component + (v_axis * static_cast<float>(j));
            vertex.normal_ = normal;
            vertex.texture_coordinate_ = math::Vec2f(u_texture_coordinate, static_cast<float>(j) / static_cast<float>(plane.height_));
...
std::unique_ptr<MeshData> MeshGenerator::GenerateBox()
{
    std::vector<Vertex> vertices;
    std::vector<uint32> indices;
    vertices.reserve(24);
...
std::unique_ptr<MeshData> MeshGenerator::GenerateTorus(const Torus& torus)
{
    std::vector<Vertex> vertices;
    std::vector<uint32> indices;
    ...
    {
        float v = static_cast<float>(i) / static_cast<float>(radial_segments);
        ...
        {
            float u = static_cast<float>(j) / static_cast<float>(tubular_segments);
...
std::unique_ptr<MeshData> MeshGenerator::GenerateCone(const Cone& cone)
{
    Cylinder cylinder{};
...
std::unique_ptr<MeshData> MeshGenerator::GenerateCylinder(const Cylinder& cylinder)
{
    std::vector<Vertex> vertices;
    std::vector<uint32> indices;
    ...
    float slope = (bottom_radius - top_radius) / static_cast<float>(cylinder.height_);
    ...
    {
        float v = static_cast<float>(y) / static_cast<float>(cylinder.height_segments_);
        ...
        {
            float u = static_cast<float>(x) / static_cast<float>(cylinder.radial_segments_);
            ...
            vertex.position_ = math::Vec3f(radius * sin_theta,
                                           -v * static_cast<float>(cylinder.height_) + half_height,
                                           radius * cos_theta);
...
std::unique_ptr<MeshData> MeshGenerator::GenerateCylinderCap(const Cylinder& cylinder, uint32& index, bool is_top_cap)
{
    std::vector<Vertex> vertices;
    std::vector<uint32> indices;

    float half_height = static_cast<float>(cylinder.height_) * 0.5F;
    float radius = is_top_cap ? cylinder.top_radius_ : cylinder.bottom_radius_;
    float sign = is_top_cap ? 1.0F : -1.0F;
