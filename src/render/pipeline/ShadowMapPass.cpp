#include "render/pipeline/ShadowMapPass.hpp"
...
const std::string ShadowMapPass::kShadowMapFragmentShaderName = "shadow_map.fragment.glsl";

ShadowMapPass::ShadowMapPass(IRenderingManager* rendering_manager)
: rendering_manager_(rendering_manager)
, cascaded_shadow_map_(Constants::kShadowCascadeCount)
{
}

void ShadowMapPass::Execute(const Camera& camera, const entt::registry& registry, const TimeDelta& time_delta)
{
...
bool ShadowMapPass::GetShadowCastingDirectionalLight(const entt::registry& registry, DirectionalLight& out_directional_light)
{
...
void ShadowMapPass::RenderEntities(const math::Matrix4x4& light_view_matrix,
                                   ...
                                   const TimeDelta& time_delta)
{
    ...
    for (Entity viewable_entity : viewable_entities)
    {
        ...
        rendering_context->BindTextureSampler(0, diffuse_map_sampler);
        uniform_manager->SetDiffuseSamplerName(shader_program, 0);
